...
**"抛出了异常"不等于"报错信息明确"**——这条规则容易被字面满足、实质空洞地执行。
例如 `raise RuntimeError(f"XX 调用失败: {e}")` 表面上看像是带了"具体原因"，但如果
`e` 本身只是一层薄的包装异常（比如 `requests.HTTPError` 默认只给
`422 Client Error: ... for url: ...` 这种 HTTP 状态行，不含服务端真正返回的
校验详情），这条报错实际上跟没写原因一样——调用方看到的还是"失败了，但不知道
为什么"。

真实教训：`backend/agents/validator_stub.py` 调用 C 的 HTTP 验证服务失败时，
`raise RuntimeError(f"C 的 HTTP 验证服务调用失败: {e}")` 表面符合"抛出明确异常"
的要求，但 `resp.raise_for_status()` 抛出的 `HTTPError` 只带 HTTP 状态行，FastAPI
真正返回的 Pydantic 校验详情（到底哪个字段、为什么不合法）从未被读取、从未被
记录——那次 422 失败后，唯一能诊断原因的信息永久丢失，只能靠事后反复构造请求去猜、
去复现（而这条流水线完全没有跨次记忆，失败时的真实上下文本来就留不住，见"记忆层"
一节，错误信息这道最后防线一旦也是空的，问题就彻底没法回溯）。已修复：改为捕获
`requests.HTTPError` 时读取 `resp.text` 一并塞进异常信息。

**正确做法**：包装/转发一个外部调用（HTTP 请求、subprocess、第三方 SDK）抛出的
异常时，不能只依赖那个异常自己的默认字符串表示，要主动取它本该携带、但默认字符串
不会显示的底层诊断信息——HTTP 请求要读 `resp.text`/`resp.json()`；subprocess 要
带上 `stdout`/`stderr`；第三方 SDK 异常要检查有没有 `.response`/`.body` 这类属性。
判断标准：**这条报错信息，能不能让下一个看到它的人（哪怕不在现场、没法复现）直接
定位到具体是哪个字段/哪一行/什么输入导致的，而不需要重新构造场景去猜**——做不到就
不算"报错信息明确"，哪怕技术上确实抛出了异常。
...
### 架构设计/重构前置动作：先查有没有成熟开源方案

在做系统架构设计、大幅重构、大幅修改，或从零搭建一个大框架/大模块之前，先主动搜索
GitHub 上是否已有成熟、维护活跃的开源项目可以参考或直接复用，不要默认"自己设计一套"
是唯一选项——本文档"技术栈合理性分析"一节列的原题建议 vs 推荐替代方案就是这种对比
的产出，应该在提出实现方案**之前**做这一步，而不是事后才补对比表。

找到候选项目后要给出真实取舍（star 数/维护活跃度/接入成本/是否匹配当前技术栈/协议
是否兼容），而不是搜到一个名字相关的项目就直接推荐；如果调研后确认没有合适的现成方案、
或者接入成本明显高于自研，也要说清楚"为什么不用现成的"，不能跳过调研这一步直接开始写。
